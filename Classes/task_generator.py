...
class TaskSystems:
    ...
    def assign_task_manual(self, employees, attendance, day, company):
        ...
        for task in self.task_list[:]:
            ...
            while True:
                name = input(
                    f"Enter an employee from the list to complete \"{task.type} - {task.name.title()}\": ").strip()
                ...
                if employee is None:
                    ...
                    if not found:
                        print(
                            "Employee is unavailable (absent, already working, or is CEO).")
                    else:
                        break
    ...
    def assign_task_manual_individual(self, emp):
        ...
        self.task_list[0].duration = random.randint(
            *self.size_lookup[self.task_list[0].size])
